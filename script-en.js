(function () {
  ...
  /* ── FULL-WIDTH GALLERIES WITH CATEGORIES ── */
  ...
  window.fwGoto = makeFwGallery(
    ...
    ['Exterior view of Villa VinoSelo', 'Pool at Villa VinoSelo', 'Summer kitchen at Villa VinoSelo', 'Wellness area with sauna and whirlpool', 'Garden at Villa VinoSelo in the evening'],
  ...
  window.fwGoto2 = makeFwGallery(
    ...
    ['Wine road of the Poreština region', 'Olive groves nearby', 'Parenzana cycle path', 'Coast of the Istrian Riviera'],
    null,
    [
      ['Wine Road', 'Family-run wineries and cellars of the Poreština region close by.', 'nearby'],
      ['Olive Groves', 'Fresh, cold-pressed olive oil straight from local producers.', 'nearby'],
      ['Parenzana Cycle Path', 'Historic railway route for cycling and hiking through the vineyards.', 'right outside'],
      ['Coast', 'Fine coves and beaches of the Istrian Riviera.', '15 min']
    ]
  );
  ...
  if (inqForm) {
    inqForm.addEventListener('submit', function (e) {
      ...
      fetch(inqForm.action, {
        ...
        if (res.ok) {
          inqStatus.textContent = 'Thank you! Your inquiry has been sent.';
          inqStatus.className = 'inq-status ok';
          inqForm.reset();
          setTimeout(closeInquiry, 2200);
        } else {
          inqStatus.textContent = 'Something went wrong. Please email us directly at info@villavinoselo.com.';
  ...
  /* ── LEAFLET MAP (load on click only – privacy, no automatic OSM request) ── */
  window.loadMap = function () {
    ...
    L.marker(vila, { icon: icon }).addTo(map).bindPopup('<b>Villa VinoSelo</b><br>Vranje Selo, Istria').openPopup();
